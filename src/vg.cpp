...
tuple<id_t, bool, id_t, bool> VG::canonical_edge_key(const Edge& edge) {
    if (edge.from() < edge.to() ||
        (edge.to() == edge.from() && !edge.from_start() && !edge.to_end())) {
        return make_tuple(edge.from(), edge.from_start(), edge.to(), edge.to_end());
    }
    else {
        return make_tuple(edge.to(), !edge.to_end(), edge.from(), !edge.from_start());
    }
}

tuple<id_t, bool, id_t, bool> VG::canonical_edge_key(const NodeSide& side1, const NodeSide& side2) {
    if (side1.node < side2.node ||
        (side1.node == side2.node && side1.is_end && !side2.is_end)) {
        return make_tuple(side1.node, !side1.is_end, side2.node, side2.is_end);
    }
    else {
        return make_tuple(side2.node, !side2.is_end, side1.node, side1.is_end);
    }
}
...
void VG::add_node(const Node& node) {
    if (!has_node(node)) {
        ...
        *graph.add_node() = node;
        ...
        auto iter = node_index.find(node.id());
        if (iter == node_index.end()) {
            // adjacency list not added by an edge previously, make a new one
            node_index[node.id()] = make_tuple<int, vector<int>, vector<int>>(graph.node_size() - 1,
                                                                              vector<int>(), vector<int>());
        }
        else {
            // adjacency list added by an edge previously, just update the index
            get<0>(iter->second) = graph.node_size() - 1;
        }
    }
}
...
void VG::index_edge(int idx) {
    // add to edge index
    const Edge& edge = graph.edge(idx);
    edge_index[canonical_edge_key(edge)] = idx;
    // add to node index
    auto from_iter = node_index.find(edge.from());
    if (from_iter == node_index.end()) {
        // add a sentinel value for the actual index
        node_index[edge.from()] = make_tuple<int, vector<int>, vector<int>>(-1, vector<int>(), vector<int>());
        from_iter = node_index.find(edge.from());
    }
    if (edge.from_start()) {
        get<1>(from_iter->second).push_back(idx);
    }
    else {
        get<2>(from_iter->second).push_back(idx);
    }
    if (!(edge.from() == edge.to() && edge.from_start() != edge.to_end())) {
        auto to_iter = node_index.find(edge.to());
        if (to_iter == node_index.end()) {
            // add a sentinel value for the actual index
            node_index[edge.to()] = make_tuple<int, vector<int>, vector<int>>(-1, vector<int>(), vector<int>());
            to_iter = node_index.find(edge.to());
        }
        if (edge.to_end()) {
            get<2>(to_iter->second).push_back(idx);
        }
        else {
            get<1>(to_iter->second).push_back(idx);
        }
    }
}
...
vector<pair<id_t, bool>> VG::edges_start(Node* node) {
    if(node == nullptr) {
        return vector<pair<id_t, bool>>();
    }
    return edges_start(node->id());
}

vector<pair<id_t, bool>> VG::edges_start(id_t id) {
    vector<pair<id_t, bool>> to_return;
    auto iter = node_index.find(id);
    if (iter == node_index.end()) {
        return to_return;
    }
    for (int i : get<1>(iter->second)) {
        const Edge& edge = graph.edge(i);
        if (edge.from() == id && edge.from_start()) {
            to_return.push_back(make_pair(edge.to(), !edge.to_end()));
        }
        else {
            to_return.push_back(make_pair(edge.from(), edge.from_start()));
        }
    }
    //cerr << "returning edges on start of " << id << " to:" << endl;
    //for (auto e : to_return) {
    //    cerr << "\t" << e.first << " " << (e.second ? "rev" : "fwd") << endl;
    //}
    return to_return;
}
...
vector<pair<id_t, bool>> VG::edges_end(id_t id) {
    ...
    //cerr << "returning edges on end of " << id << " to:" << endl;
    //for (auto e : to_return) {
    //    cerr << "\t" << e.first << " " << (e.second ? "rev" : "fwd") << endl;
    //}
    return to_return;
...
int VG::start_degree(Node* node) {
    return get<1>(node_index[node->id()]).size();
}

int VG::end_degree(Node* node) {
    return get<2>(node_index[node->id()]).size();
}
...
void VG::edges_of_node(Node* node, vector<Edge*>& edges) {
    auto& node_record = node_index[node->id()];
    for (int i : get<1>(node_record)) {
        edges.push_back(graph.mutable_edge(i));
    }
    for (int i : get<2>(node_record)) {
...
void VG::remove_non_path(void) {
    ...
    // convert away from pointers so that they don't get invalidated once
    // we start changing the underlying graph
    vector<pair<NodeSide, NodeSide>> nodesides_of_non_path_edges;
    nodesides_of_non_path_edges.reserve(non_path_edges.size());
    for (Edge* e : non_path_edges) {
        nodesides_of_non_path_edges.push_back(NodeSide::pair_from_edge(e));
    }
    
    // and destroy them
    for (auto& e : nodesides_of_non_path_edges) {
        destroy_edge(e);
    }
...
void VG::remove_path(void) {
    ...
    // convert away from pointers so that they don't get invalidated once
    // we start changing the underlying graph
    vector<pair<NodeSide, NodeSide>> nodesides_of_path_edges;
    nodesides_of_path_edges.reserve(path_edges.size());
    for (Edge* e : path_edges) {
        nodesides_of_path_edges.push_back(NodeSide::pair_from_edge(e));
    }
    
    // and destroy them
    for (auto& e : nodesides_of_path_edges) {
        destroy_edge(e);
    }
...
Node* VG::concat_nodes(const list<NodeTraversal>& nodes) {
    ...
    // collect the IDs of the nodes we want to destroy
    vector<id_t> destroy_node_ids;
    destroy_node_ids.reserve(nodes.size());
    for (auto& n : nodes) {
        destroy_node_ids.push_back(n.node->id());
    }
    
    // and actually destroy them
    for (auto node_id : destroy_node_ids) {
...
Node* VG::merge_nodes(const list<Node*>& nodes) {
    ...
    vector<id_t> destroy_ids;
    for (auto& m : nodes) {
        if (m != n) {
            destroy_ids.push_back(m->id());
        }
    }
    for (id_t id : destroy_ids) {
        destroy_node(id);
    }
...
void VG::build_node_index(void) {
    if (!node_index.empty()) {
        clear_node_index();
    }
    for (int i = 0; i < graph.node_size(); ++i) {
        node_index[graph.node(i).id()] = make_tuple(i, vector<int>(), vector<int>());
    }
}

void VG::build_edge_index(void) {
    if (!edge_index.empty()) {
        clear_edge_index();
    }
...
void VG::clear_node_index(void) {
    node_index.clear();
}

void VG::clear_node_index_no_resize(void) {
...
void VG::clear_adjacency_index(void) {
    if (!node_index.empty()) {
        for (auto& node_record : node_index) {
            get<1>(node_record.second).clear();
            get<2>(node_record.second).clear();
        }
    }
}

void VG::clear_edge_index(void) {
    edge_index.clear();
    clear_adjacency_index();
}

void VG::clear_edge_index_no_resize(void) {
#ifdef USE_DENSE_HASH
...
#ifdef USE_DENSE_HASH
    clear_node_index_no_resize();
    clear_edge_index_no_resize();
#else
    clear_indexes();
#endif
}
...
void VG::rebuild_edge_index(void) {
    clear_edge_index_no_resize();
    build_edge_index();
}
...
bool VG::has_node(id_t id) {
    auto iter = node_index.find(id);
    return iter == node_index.end() ? false : get<0>(iter->second) != -1;
}
  ...
  int namespace_end = name.find_last_of("/#");
	...
	if (has_node(id)){
	   return get_node(id);
		...
		new_node->set_id(id);
        node_index[new_node->id()] = make_tuple<int, vector<int>, vector<int>>(graph.node_size()-1,
                                                                               vector<int>(),
                                                                               vector<int>());
		return new_node;
	}
...
bool VG::has_edge(Edge* edge) {
    return edge ? has_edge(*edge) : false;
}

bool VG::has_edge(const Edge& edge) {
    return edge_index.find(canonical_edge_key(edge)) != edge_index.end();
}

bool VG::has_edge(const NodeSide& side1, const NodeSide& side2) {
    return edge_index.find(canonical_edge_key(side1, side2)) != edge_index.end();
}
...
void VG::remove_duplicated_in(VG& g) {
    vector<id_t> nodes_to_destroy;
    for (id_t i = 0; i < graph.node_size(); ++i) {
        Node* n = graph.mutable_node(i);
        if (g.has_node(n)) {
            nodes_to_destroy.push_back(n->id());
        }
    }
    vector<pair<NodeSide, NodeSide>> edges_to_destroy;
    for (id_t i = 0; i < graph.edge_size(); ++i) {
        Edge* e = graph.mutable_edge(i);
        if (g.has_edge(e)) {
            edges_to_destroy.push_back(NodeSide::pair_from_edge(e));
        }
    }
    ...
    for (pair<NodeSide, NodeSide>& e : edges_to_destroy) {
...
void VG::remove_duplicates(void) {
    ...
    for (id_t i = 0; i < graph.edge_size(); ++i) {
        Edge* e = graph.mutable_edge(i);
        auto pair_edge = NodeSide::pair_from_edge(*e);
        auto f = edge_counts.find(pair_edge);
        if (f != edge_counts.end()
            && f->second > 1) {
            --f->second;
            edges_to_destroy.push_back(pair_edge);
        }
    }
    for (auto iter = edges_to_destroy.begin(); iter != edges_to_destroy.end(); iter++) {
        // Find and destroy the edge that does the same thing in g.
        destroy_edge(*iter);
    }
}
...
void VG::swap_node_id(id_t node_id, id_t new_id) {
    swap_node_id(graph.mutable_node(get<0>(node_index[node_id])), new_id);
}

void VG::swap_node_id(Node* node, id_t new_id) {
    cerr << "##" << endl << "SWAPPING NODE ID" << endl;
    id_t old_id = node->id();
    node->set_id(new_id);

    // we check if the old node exists, and bail out if we're not doing what we expect
    assert(node_index.find(new_id) == node_index.end());

    // otherwise move to a new id
    node_index[new_id] = node_index[old_id];
    node_index.erase(old_id);
    auto& node_record = node_index[new_id];
    
    auto update_edge_list = [&](vector<int> edge_idxs) {
        for (int i : edge_idxs) {
            Edge* edge = graph.mutable_edge(i);
            auto old_key = canonical_edge_key(*edge);
            if (edge->from() == old_id) {
                edge->set_from(new_id);
            }
            if (edge->to() == old_id) {
                edge->set_to(new_id);
            }
            auto iter = edge_index.find(old_key);
            if (iter != edge_index.end()) {
                edge_index.erase(iter);
            }
            edge_index[canonical_edge_key(*edge)] = i;
        }
    };
    
    update_edge_list(get<1>(node_record));
    update_edge_list(get<2>(node_record));
}
...
void VG::from_gfa(istream& in, bool showp) {
    ...
    for (auto it = name_to_seq.begin(); it != name_to_seq.end(); it++){
...
void VG::bluntify(void) {
    ...
    set<pair<NodeSide, NodeSide>> bad_edges;
        ...
        if (edge->overlap() > 0) {
            ...
            if (from_overlap != to_overlap) {
                ...
                if (aln.path().mapping(0).edit_size() <= 2
                    ...
                    // Drop the edge
                    bad_edges.insert(NodeSide::pair_from_edge(edge));
                }
    ...
    for_each_edge([&](Edge* edge) {
        ...
        if (edge->overlap() > 0) {
            ...
            overlapped_nodes.insert(left_node->id());
            overlapped_nodes.insert(right_node->id());
            ...
            overlap_edges.emplace_back(left, right);
...
void VG::sort(void) {
    ...
    size_t seen_root = 0;
    for (size_t i = 0; i < graph.node_size(); i++) {
        // get the next node in the order
        NodeTraversal n = sorted_nodes[i + seen_root];
        // if we run into the root node in the ordering, skip it and mark the
        // offset between the indexing of the two vectors
        if (n.node == root) {
            seen_root = 1;
            n = sorted_nodes[i + seen_root];
        }
        
        // put the indicated node into this position in the graph
...
int VG::node_rank(Node* node) {
    return node_rank(node->id());
}

// returns the rank of the node in the protobuf array that backs the graph
int VG::node_rank(id_t id) {
    return get<0>(node_index[id]);
}
...
void VG::swap_nodes(Node* a, Node* b) {
    auto& node_a_record = node_index[a->id()];
    auto& node_b_record = node_index[b->id()];
    graph.mutable_node()->SwapElements(get<0>(node_a_record), get<0>(node_b_record));
    std::swap(get<0>(node_a_record), get<0>(node_b_record));
}
...
Edge* VG::create_edge(id_t from, id_t to, bool from_start, bool to_end) {
    ...
    index_edge(graph.edge_size()-1);
    return edge;
}

Edge* VG::get_edge(const NodeSide& side1, const NodeSide& side2) {
    auto key = canonical_edge_key(side1, side2);
    auto iter = edge_index.find(key);
    return iter != edge_index.end() ? graph.mutable_edge(iter->second) : nullptr;
}
...
void VG::destroy_edge(const NodeSide& side1, const NodeSide& side2) {
    Edge* edge = get_edge(side1, side2);
    if (!edge) {
        return;
    }
    
    unindex_edge_for_traversal(edge);
    
    auto key = canonical_edge_key(*edge);
    int idx = edge_index[key];
    edge_index.erase(key);
    std::swap(*graph.mutable_edge(idx), *graph.mutable_edge(graph.edge_size() - 1));
    graph.mutable_edge()->ReleaseLast();
    
    // update the index of the edge we swapped into this position (unless this edge was the last one
    // in the array, so no swap actually took place)
    if (idx < graph.edge_size()) {
        const Edge& swapped_edge = graph.edge(idx);
        edge_index[canonical_edge_key(swapped_edge)] = idx;
        
        vector<int>& from_idxs = swapped_edge.from_start() ? get<1>(node_index[swapped_edge.from()]) : get<2>(node_index[swapped_edge.from()]);
        for (int i = 0; i < from_idxs.size(); i++) {
            if (from_idxs[i] == graph.edge_size()) {
                from_idxs[i] = idx;
                break;
            }
        }
        
        if (swapped_edge.from() == swapped_edge.to() && swapped_edge.from_start() != swapped_edge.to_end()) {
            return;
        }
        
        vector<int>& to_idxs = swapped_edge.to_end() ? get<2>(node_index[swapped_edge.to()]) : get<1>(node_index[swapped_edge.to()]);
        for (int i = 0; i < to_idxs.size(); i++) {
            if (to_idxs[i] == graph.edge_size()) {
                to_idxs[i] = idx;
                break;
            }
        }
...
void VG::unindex_edge_for_traversal(const NodeSide& side1, const NodeSide& side2) {
    unindex_edge_for_traversal(get_edge(side1, side2));
}

void VG::unindex_edge_for_traversal(Edge* edge) {
    ...
    int i;
    vector<int>& from_idxs = edge->from_start() ? get<1>(node_index[edge->from()]) : get<2>(node_index[edge->from()]);
    for (i = 0; i < from_idxs.size(); i++) {
        if (key == canonical_edge_key(graph.edge(from_idxs[i]))) {
            break;
        }
    }
    std::swap(from_idxs[i], from_idxs.back());
    from_idxs.pop_back();
    
    // don't try to delete a reversing self edge twice
    if (edge->from() == edge->to() && edge->from_start() != edge->to_end()) {
        return;
    }
    
    vector<int>& to_idxs = edge->to_end() ? get<2>(node_index[edge->to()]) : get<1>(node_index[edge->to()]);
    for (i = 0; i < to_idxs.size(); i++) {
        if (key == canonical_edge_key(graph.edge(to_idxs[i]))) {
            break;
        }
    }
    std::swap(to_idxs[i], to_idxs.back());
    to_idxs.pop_back();
}

void VG::index_edge_for_traversal(Edge* edge) {

    int idx = edge_index[canonical_edge_key(*edge)];
    vector<int>& from_idxs = edge->from_start() ? get<1>(node_index[edge->from()]) : get<2>(node_index[edge->from()]);
    from_idxs.push_back(idx);
    ...
    vector<int>& to_idxs = edge->to_end() ? get<2>(node_index[edge->to()]) : get<1>(node_index[edge->to()]);
    to_idxs.push_back(idx);
}

Node* VG::get_node(id_t id) {
    auto iter = node_index.find(id);
    return iter == node_index.end() ? nullptr : graph.mutable_node(get<0>(iter->second));
}
...
void VG::destroy_node(id_t id) {
    ...
    auto& node_record = node_index[id];
    
    for (vector<int>* edge_list : {&get<1>(node_record), &get<2>(node_record)}) {
        for (int i : *edge_list) {
            // make sure we didn't already delete this as a self edge
            if (i >= graph.edge_size()) {
                continue;
            }
            const Edge& edge = graph.edge(i);
            
            // another check that we didn't already delete this as a self edge
            if (edge.to() != id && edge.from() != id) {
                continue;
            }
            
            auto key = canonical_edge_key(edge);
            // delete the other side of this edge if it's not a self edge
            if (edge.from() != edge.to()) {
                if (edge.from() == id) {
                    auto& other_node_record = node_index[edge.to()];
                    vector<int>& to_idxs = edge.to_end() ? get<2>(other_node_record) : get<1>(other_node_record);
                    for (int j = 0; j < to_idxs.size(); j++) {
                        if (to_idxs[j] == i) {
                            std::swap(to_idxs[j], to_idxs.back());
                            to_idxs.pop_back();
                            break;
                        }
                    }
                    ...
                    auto& other_node_record = node_index[edge.from()];
                    vector<int>& from_idxs = edge.from_start() ? get<1>(other_node_record) : get<2>(other_node_record);
                    for (int j = 0; j < from_idxs.size(); j++) {
                        if (from_idxs[j] == i) {
                            std::swap(from_idxs[j], from_idxs.back());
                            from_idxs.pop_back();
                            break;
                        }
                    }
            ...
            // remove and unindex the edge
            std::swap(*graph.mutable_edge(i), *graph.mutable_edge(graph.edge_size() - 1));
            graph.mutable_edge()->ReleaseLast();
            edge_index.erase(key);
            
            // update the indexing of the edge we swapped into this position
            if (i != graph.edge_size()) {
                const Edge& swapped_edge = graph.edge(i);
                
                edge_index[canonical_edge_key(swapped_edge)] = i;
                
                auto& from_node_record = node_index[edge.from()];
                vector<int>& from_idxs = edge.from_start() ? get<1>(from_node_record) : get<2>(from_node_record);
                for (int j = 0; j < from_idxs.size(); j++) {
                    if (from_idxs[j] == graph.edge_size()) {
                        from_idxs[j] = i;
                        break;
                    }
                }
                
                // don't update in the reverse direction if this is a reversing self loop because
                // the update already happened in the previous step
                if (!(swapped_edge.from() == swapped_edge.to() && swapped_edge.from_start() != swapped_edge.to_end())) {
                    auto& to_node_record = node_index[edge.to()];
                    vector<int>& to_idxs = edge.to_end() ? get<2>(to_node_record) : get<1>(to_node_record);
                    for (int j = 0; j < to_idxs.size(); j++) {
                        if (to_idxs[j] == graph.edge_size()) {
                            to_idxs[j] = i;
                            break;
                        }
                    }
    ...
    int node_idx = get<0>(node_record);
    node_index.erase(id);
    std::swap(*graph.mutable_node(node_idx), *graph.mutable_node(graph.node_size() - 1));
    graph.mutable_node()->ReleaseLast();
    if (node_idx != graph.node_size()) {
        get<0>(node_index[graph.node(node_idx).id()]) = node_idx;
    }
}
...
#pragma omp critical (cerr)
    ...
    new_node_ids.push_back(last_node->id());
...
#pragma omp critical (cerr)
    {
        for(id_t node_id : new_node_ids) {
            Node* part = get_node(node_id);
    ...
    for(auto e : edges_start(node)) {
        ...
        if (e.first == node->id()) {
            ...
            if (e.second) {
                ...
                e.first = new_node_ids.front();
            } else {
                // otherwise, it's from the end, so set it to the back node
                e.first = new_node_ids.back();
            }
        }
        // Make an edge to the left node's start from wherever this edge went.
        edges_to_create.emplace(make_pair(e.first, e.second), make_pair(new_node_ids.front(), false));
    }
    ...
    for(auto e : edges_end(node)) {
        ...
        edges_to_create.emplace(make_pair(new_node_ids.back(), false), make_pair(e.first, e.second));
    }
...
#pragma omp critical (cerr)
            ...
            for(int i = 0; i < positions.size(); i++) {
                ...
                chunk.mutable_position()->set_node_id(new_node_ids[i]);
            ...
            remainder.mutable_position()->set_node_id(new_node_ids.back());
...
#pragma omp critical (cerr)
    ...
    destroy_node(dividing_id);
    
    for (id_t node_id : new_node_ids) {
        parts.push_back(get_node(node_id));
    }
}
...
bool VG::is_valid(bool check_nodes,
    ...
    if (check_nodes) {
        ...
        for (int i = 0; i < graph.node_size(); ++i) {
            Node* n = graph.mutable_node(i);
            if (node_index.find(n->id()) == node_index.end()) {
    ...
    if (check_edges) {
        if (edge_index.size() != graph.edge_size()) {
            cerr << "graph invalid: edge count is not equal to that found in edge index" << endl;
            return false;
        }
        ...
        for (int i = 0; i < graph.edge_size(); ++i) {
            ...
            if (node_index.find(f) == node_index.end()) {
                cerr << "graph invalid: edge index=" << i
                     << " (" << f << "->" << t << ") cannot find node (from) " << f << endl;
                return false;
            }
            if (node_index.find(t) == node_index.end()) {
                cerr << "graph invalid: edge index=" << i
                     << " (" << f << "->" << t << ") cannot find node (to) " << t << endl;
                return false;
            }

            if (!edge_index.count(canonical_edge_key(*e))) {
                // todo check if it's in the vector
                cerr << "graph invalid: edge index=" << i << " " << pb2json(*e) << endl;
                return false;
            }
        }
        ...
        for (int i = 0; i < graph.node_size(); i++) {
            Node* n = graph.mutable_node(i);
            if (node_index.find(n->id()) == node_index.end()) {
                cerr << "graph invalid: node index=" << i << " " << pb2json(*n) << endl;
                return false;
            }
            auto node_record = node_index[n->id()];
            if (get<0>(node_record) != i) {
                cerr << "graph invalid: node index=" << i << " mismatches index" << endl;
                return false;
            }
            bool on_start = true;
            for (vector<int>* edge_list : {&get<1>(node_record), &get<2>(node_record)}) {
                for (int j : *edge_list) {
                    ...
                    if (!((edge.from() == n->id() && edge.from_start() == on_start)
                          || (edge.to() == n->id() && edge.to_end() != on_start))) {
                        cerr << "graph invalid: node index=" << i << " edge does not touch node" << endl;
                        return false;
                    }
                }
                on_start = false;
...
void VG::for_each_gcsa_kmer_position_parallel(int kmer_size, bool path_only,
    ...
    if(edges.empty()) {
        ...
        // pointer may have been invalidated if we're not using the local copy
        if (has_node(tail_id)) {
            tail_node = get_node(tail_id);
        }
...
#pragma omp single
    ...
    for(id_t node_id : to_destroy) {
        ...
        for(auto path_and_mapping : paths.get_node_mapping(get_node(node_id))) {
...
void VG::print_indexes(void) {
    cerr << endl << "NODE" << endl;
    for (auto& node_record : node_index) {
        cerr << "\t" << node_record.first << "\t" << get<0>(node_record.second) << endl;
        cerr << "\t\tstart:\t";
        for (int i : get<1>(node_record.second)) {
            cerr << i << " ";
        }
        cerr << endl;
        cerr << "\t\tend:\t";
        for (int i : get<2>(node_record.second)) {
            cerr << i << " ";
        }
        cerr << endl;
    }
    cerr << "EDGE" << endl;
    for (auto& edge_record : edge_index) {
        cerr << "\t" << get<0>(edge_record.first) << " " << get<1>(edge_record.first) << " " << get<2>(edge_record.first) << " " << get<3>(edge_record.first) << "\t\t" << edge_record.second << endl;
    }
    cerr << "GRAPH" << endl;
    for (int i = 0; i < graph.node_size(); i++) {
        cerr << i << " " << pb2json(graph.node(i)) << endl;
    }
    for (int i = 0; i < graph.edge_size(); i++) {
        cerr << i << " " << pb2json(graph.edge(i)) << endl;
    }
}
...
*/
void VG::topological_sort(vector<NodeTraversal>& order) {
    //assert(is_valid());
...
#pragma omp critical (cerr)
    ...
    unordered_map<id_t, pair<size_t, size_t>> side_degrees;
    
    // the node IDs in the graph
    vector<id_t> node_ids;
    node_ids.reserve(graph.node_size());
    ...
    for (size_t i = 0; i < graph.node_size(); i++) {
        ...
        auto& node_record = node_index[id];
        side_degrees[id] = make_pair(get<1>(node_record).size(), get<2>(node_record).size());
    }
    
    // sort the node IDs to ensure stable sort across different systems and regardless of
    // internal sorting of Protobuf graph
    std::sort(node_ids.begin(), node_ids.end());
    
    // find the nodes with 0 in degree an initialize the queue with them
    list<NodeTraversal> heads;
    for (id_t id : node_ids) {
        if (side_degrees[id].first == 0) {
            heads.emplace_back(get_node(id), false);
            side_degrees.erase(id);
        }
    }
    
    // iterate across nodes in the system-independent order
    size_t id_idx = 0;
    while (id_idx < node_ids.size()) {
        
        if (heads.empty()) {
            // we haven't found any new heads to start a traversal with, so check if the next node
            // has been ordered yet
            
            id_t node_id = node_ids[id_idx];
            id_idx++;
            
            if (side_degrees.count(node_id)) {
                // this node has not yet been ordered, it must be part of a cycle, which we arbitrarily
                // break here
                
                // add it the queue and mark it as queued
                heads.emplace_back(get_node(node_id), false);
                side_degrees.erase(node_id);
                
                // remove the edges going into this traversal
                vector<NodeTraversal> prevs;
                nodes_prev(heads.back(), prevs);
                for (NodeTraversal& prev : prevs) {
                    // check whether this node has been added to the ordering already
                    auto iter = side_degrees.find(prev.node->id());
                    if (iter != side_degrees.end()) {
                        // reduce the degree of the appropriate side
                        size_t& inward_degree = prev.backward ? iter->second.first : iter->second.second;
                        inward_degree--;
                        if (inward_degree == 0) {
                            // after removing this edge, the node is now a head, add it to the queue
                            heads.push_back(prev.reverse());
                            side_degrees.erase(iter);
                        }
                    }
                }
            }
            else {
                // this node has been added to the ordering, proceed to checking the next node
                continue;
            }
        }
        ...
        while (!heads.empty()) {
            ...
            NodeTraversal head_trav = heads.front();
            heads.pop_front();
            
            // add it to the topological order
            order.push_back(head_trav);
            
            // remove its outgoing edges
            vector<NodeTraversal> nexts;
            nodes_next(head_trav, nexts);
            for (NodeTraversal& next : nexts) {
                // we need to check whether the next node has already been added to the queue,
                // which is possible in cycles
                auto iter = side_degrees.find(next.node->id());
                if (iter != side_degrees.end()) {
                    // reduce the degree of the appropriate side
                    size_t& inward_degree = next.backward ? iter->second.second : iter->second.first;
                    inward_degree--;
                    if (inward_degree == 0) {
                        // after removing this edge, the node is now a head, add it to the queue
...
//        });
//    
//    // to keep track of which edges we've already traversed
//    unordered_set<Edge*> visited_edges;
//
...
//                Edge* edge = get_edge(prev_node, n);
...
#pragma omp critical (cerr)
    ...
    for(pair<NodeSide, NodeSide>& edge : to_remove) {
#ifdef debug
#pragma omp critical (cerr)
        cerr << "Removed cycle edge " << edge.first << "->" << edge.second << endl;
#endif
        destroy_edge(edge);
    }
